...
if __name__=='__main__':
    ...
    raw_pcd = make_cloud(
        tree_preproc.item().get('raw_pcd_points'),
    ...
    if args.method in ['default', 'field']:
        for voxel_size in np.arange(0.01, 0.1, 0.001):
            likelihood_map_pcd = make_cloud(likelihood_points, likelihood_colors)
            ...
            likelihood_map_points = np.asarray([
                likelihood_map_vg.origin + pt.grid_index * likelihood_map_vg.voxel_size
                for pt in likelihood_map_vg.get_voxels()
            ])
            likelihood_map_values = np.asarray([
                pt.color[0] for pt in likelihood_map_vg.get_voxels()
            ])
        ...
        likelihood_map_pcd = make_cloud(likelihood_map_points, likelihood_map_colors)
